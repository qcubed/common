...
class DateTimeTests extends \QCubed\Test\UnitTestCaseBase
{
    ...
    public function testLimitedConstructors()
    {
        $dt1 = QDateTime::Now();
        $ts = $dt1->Timestamp;
        $strIso = $dt1->qFormat(QDateTime::FORMAT_ISO_COMPRESSED);

        $dt2 = new QDateTime($dt1, null, QDateTime::DATE_AND_TIME_TYPE);
        $dt3 = new QDateTime ($dt1);
        $this->assertTrue($dt2->IsEqualTo($dt3));

        $dt2 = new QDateTime('@' . $ts, null, QDateTime::DATE_AND_TIME_TYPE);
        $this->assertTrue($dt2->IsEqualTo($dt1));
        $this->assertFalse($dt2->IsDateNull());
        $this->assertFalse($dt2->IsTimeNull());

        $dt2 = new QDateTime('@' . $ts, null, QDateTime::DATE_ONLY_TYPE);
        $this->assertFalse($dt2->IsDateNull());
        $this->assertTrue($dt2->IsTimeNull());

        $dt2 = new QDateTime('@' . $ts, null, QDateTime::TIME_ONLY_TYPE);
        $this->assertTrue($dt2->IsDateNull());
        $this->assertFalse($dt2->IsTimeNull());

        $dt2 = new QDateTime($strIso, null, QDateTime::DATE_AND_TIME_TYPE);
        $this->assertTrue($dt2->IsEqualTo($dt1));
        $this->assertFalse($dt2->IsDateNull());
        $this->assertFalse($dt2->IsTimeNull());

        $dt2 = new QDateTime($strIso, null, QDateTime::DATE_ONLY_TYPE);
        $this->assertFalse($dt2->IsDateNull());
        $this->assertTrue($dt2->IsTimeNull());

        $dt2 = new QDateTime($strIso, null, QDateTime::TIME_ONLY_TYPE);
        $this->assertTrue($dt2->IsDateNull());
        $this->assertFalse($dt2->IsTimeNull());
        ...
        $dt2 = new QDateTime(null, null, QDateTime::DATE_ONLY_TYPE);
        $this->assertTrue($dt2->IsDateNull());
        $this->assertTrue($dt2->IsTimeNull());

        $dt2 = new QDateTime(null, null, QDateTime::TIME_ONLY_TYPE);
        $this->assertTrue($dt2->IsDateNull());
        $this->assertTrue($dt2->IsTimeNull());

        $dt2 = new QDateTime(null, null, QDateTime::DATE_AND_TIME_TYPE);    // forcing it to have a date and time
        $this->assertFalse($dt2->IsDateNull());
        $this->assertFalse($dt2->IsTimeNull());
    ...
    public function testTimeZoneIssues()
    {
        ...
        $dt2 = new QDateTime ($dt1, null, QDateTime::DATE_ONLY_TYPE);
        ...
        $dt2 = new QDateTime ($dt1->format(QDateTime::ISO8601), null, QDateTime::DATE_ONLY_TYPE);
        $dt2->setTime(7, 0, 0);
        $this->assertEquals(7, $dt2->Hour);
        ...
        $this->assertEquals(1288486753,
            $dt2->Timestamp); // this isn't always true. If this is a dst boundary, it will not be true. Just making sure it is true when its supposed to be
    ...
    public function testDateAndTimeConstructor() {
        $tz = new DateTimeZone('America/Los_Angeles');
        $dt1 = new \QCubed\QDateTime ('11/1/14', $tz, \QCubed\QDateTime::DATE_ONLY_TYPE); // dst date
        $dt2 = new \QCubed\QDateTime ('7:00', $tz, \QCubed\QDateTime::TIME_ONLY_TYPE);
...
*/
    ...
    public function testSetProperties()
    {
        ...
        $obj5 = new QDateTime ('13:15:02', null, QDateTime::TIME_ONLY_TYPE);
