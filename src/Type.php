...
class Type {
	...
	private static function CastValueTo($mixItem, $strNewType) {
		...
		switch (Type::TypeFromDoc($strNewType)) {
			...
			case Type::Integer:
				if($strOriginalType == Type::Boolean)
					throw new InvalidCast(sprintf('Unable to cast %s value to %s: %s', $strOriginalType, $strNewType, $mixItem));
				if (strlen($mixItem) == 0)
					return null;
			...
			default:
				throw new InvalidCast(sprintf('Unable to cast %s value to unknown type %s', $strOriginalType, $strNewType));
		}
	}
	...
	public final static function Constant($strType) {
		switch ($strType) {
			case Type::Object: return 'Type::Object';
		...
		switch (strtolower($strType)) {
			...
			default:
				try {
					...
					throw new InvalidCast(sprintf('Unable to determine type of item from PHPDoc Comment to lookup its Type or Class: %s', $strType));
				}
		}
	}
