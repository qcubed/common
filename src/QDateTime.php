...
//			$strArray = preg_split('/([^D^M^Y^h^m^s^z^t])+/', $strFormat);
	...
	public function jsonSerialize() {
		...
		if ($this->blnTimeNull) {
			return [QCubed::JSON_OBJECT_TYPE=>'qDateTime', 'year'=>$dt->Year, 'month'=>$dt->Month - 1,
				'day'=>$dt->Day];
		...
		switch ($strName) {
			...
			case 'Age':
				...
				if ($dtsFromCurrent->IsPositive())
					return $dtsFromCurrent->SimpleDisplay() . Translator::translate(' from now');

				// It's in the past ('about 5 hours ago')
				else if ($dtsFromCurrent->IsNegative()) {
					$dtsFromCurrent->Seconds = abs($dtsFromCurrent->Seconds);
					return $dtsFromCurrent->SimpleDisplay() . Translator::translate(' ago');

				// It's current
				} else
					return Translator::translate('right now');
